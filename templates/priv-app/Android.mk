...
#~ LOCAL_APPS += $(subst $(LOCAL_PATH)/,,$(wildcard $(LOCAL_PATH)/**/*$(COMMON_ANDROID_PACKAGE_SUFFIX)))
#~ LOCAL_APPS += $(subst $(LOCAL_PATH)/,,$(wildcard $(LOCAL_PATH)/**/**/*$(COMMON_ANDROID_PACKAGE_SUFFIX)))
#~ LOCAL_APPS := $(patsubst $(LOCAL_PATH)/%, %, $(wildcard folder/**/*$(COMMON_ANDROID_PACKAGE_SUFFIX)))
#~ LOCAL_APPS := $(shell find $(call my-dir) -type f ! -path '*.git*' -name "*$(COMMON_ANDROID_PACKAGE_SUFFIX)*")
...
LOCAL_SRC_FILES := $(1)
#~ LOCAL_SRC_FILES := $(LOCAL_APPS:$(LOCAL_PATH)/%=%)
